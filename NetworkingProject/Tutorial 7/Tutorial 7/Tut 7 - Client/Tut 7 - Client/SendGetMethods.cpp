...
bool Client::SendCircle(Circle& _circle)
{
	//SEND PACKET TYPE
	if (!SendPacketType(P_CircleData))
		return false;
	...
	if (!SendInt(bufferLength))
		return false;
	//SEND OBJECT DATA
	if (!sendall((char*)&_circle, bufferLength))
		return false;
	return true;
}
...
bool Client::GetCircle(Circle & _circle)
{
	int bufferLength = sizeof(Circle); //set size of the buffer to be size of circle object
	//RECEIVE BUFFER LENGTH
	if (!GetInt(bufferLength))
		return false;
	Circle* tempCircle = new Circle(); //create a circle object that will hold received data
	//RECEIVE OBJECT
	if (!recvall((char*)tempCircle, bufferLength)) //receive circle data giving in the temporary object and the length of buffer
	{
		delete tempCircle; //if fail delete newly created object and return false
		return false;
	}
